...
func Test_TsTypes(t *testing.T) {
	for _, test := range tsTypeTests {
		...
		// Output
		v.out = true
		assert.Equal(t, test.expectedOutput, tsType("", "", v, parsedDoc{}, nil, false, false, false))

		// Input
		v.out = false
		assert.Equal(t, test.expectedInput, tsType("", "", v, parsedDoc{}, nil, false, true, false))
	}
}
...
func Test_Issue130(t *testing.T) {
	...
	assert.Equal(t, "string", tsType("", "", &variable{
		...
		schema: schema,
		out:    true,
	}, parsedDoc{}, nil, false, false, false))

	assert.Equal(t, "pulumi.Input<string>", tsType("", "", &variable{
		name:   "condition",
		schema: schema,
		out:    false,
	}, parsedDoc{}, nil, false, true, false))
}
